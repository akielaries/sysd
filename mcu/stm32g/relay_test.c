...
void clear_bits(volatile uint32_t *reg, uint32_t mask) { *reg &= ~mask; }

void set_bits(volatile uint32_t *reg, uint32_t mask) { *reg |= mask; }
...
void blink() {
  ...
  */
  while (1) {
    ...
    // LED. The BSRR register for that purpose is at offset 0x18 (24) as
    // described in section 9.4.7. We need to set bit 13
    // to make PC13 high and bit 37 (pin + 0x18) to make it low
    // respectively.
